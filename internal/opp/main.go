...
func countSavedPackets(addr string, tl *trafficlog.TrafficLog) (int, error) {
	...
	if err := tl.WritePcapng(buf); err != nil {
		return 0, fmt.Errorf("failed to write packets to pcapng: %w", err)
	}
	pcapR, err := pcapgo.NewNgReader(buf, pcapgo.NgReaderOptions{})
	if err != nil {
		return 0, fmt.Errorf("failed to initialize pcap reader: %w", err)
	}
	count := 0
	for {
...
func fillBuffers(tl *trafficlog.TrafficLog) (packetCount int, err error) {
	...
	if err != nil {
		return 0, fmt.Errorf("failed to create test network connection: %w", err)
	}

	if err := tl.UpdateAddresses([]string{blackHoleAddr}); err != nil {
		return 0, fmt.Errorf("failed to log test traffic: %w", err)
	}
	...
	for currentCount > lastCount {
		...
		for i := 0; i < 100; i++ {
			if _, err := conn.Write(payload); err != nil {
				return 0, fmt.Errorf("failed to write to listener: %w", err)
			}
		}

		select {
		case err := <-tl.Errors():
			return 0, fmt.Errorf("capture error: %w", err)
		default:
		}
		...
		if err != nil {
			return 0, fmt.Errorf("failed to count packets in save buffer: %w", err)
		}
	}
...
func memstatsFor(packetSize, bufferSize int, separateProcess bool) (*memStats, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to fill buffers: %w", err)
	}
