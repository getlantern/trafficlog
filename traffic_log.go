...
type ErrorMalformedAddress struct {
	cause error
}

// Unwrap allows for Go 1.13-style error unwrapping.
func (e ErrorMalformedAddress) Unwrap() error {
	return e.cause
}

func (e ErrorMalformedAddress) Error() string {
	return fmt.Sprintf("malformed address: %v", e.cause)
}
...
func (tl *TrafficLog) WritePcapng(w io.Writer) error {
	...
	tl.saveBuffer.forEach(func(item bufferItem) {
		...
		if err != nil {
			numErrors++
			lastError = fmt.Errorf("failed to register interface: %w", err)
			return
		}
		...
		if err := pcapW.WritePacket(gopacketCI, pkt.dataBuf.Bytes()); err != nil {
			numErrors++
			lastError = fmt.Errorf("failed to write packet: %w", err)
			return
		}
	})
	...
	if numErrors > 0 {
		return fmt.Errorf("%d errors writing packets; last error: %w", numErrors, lastError)
	}
	return nil
}
